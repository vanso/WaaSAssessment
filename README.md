# WaaSAssessment

WaaSAssessment is a PowerShell module.

Gets WaaS Assessment Platform information.

The WaaS Assessment Platform provide the following information:

- If a device is on the latest Microsoft updates.
- Whether the device has reached end of support.
- The release times for the latest applicable updates for the device.
- An assessment of why a device is not up-to-date and the potential impact it may have on the device.

Source : [About the WaaS Assessment Platform](https://learn.microsoft.com/en-us/windows/win32/sysinfo/about-update-assessor-service)

## Requirements

- Windows PowerShell 5.1
- PowerShell 7.x

## Supported platforms

- Windows

## Installation

```powershell
PS> Install-Module -Name WaaSAssessment
```

## Usage
```powershell
Get-OSUpdateAssessment
```

## License

This module is released under the terms of the GNU General Public License (GPL), Version 2.
